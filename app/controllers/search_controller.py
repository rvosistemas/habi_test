...
class SearchController:
    ...
    def get_search(property_filters, status_history_filters, status_filter) -> list:
        """
        Realiza una búsqueda de propiedades con los filtros especificados, pero se debe cumplir que el estado (status),
        este en los siguientes ["pre_venta", "en_venta", "vendido"].

        Args:
            property_filters (dict): Filtros para propiedades.
            status_history_filters (dict): Filtros para historial de estados.
            status_filter (dict): Filtro para estado.

        Returns:
            list: Una lista de resultados de búsqueda como diccionarios.
        """
        accepted_status = ["pre_venta", "en_venta", "vendido"]
        results_dict = []

        if status_filter["name"] in accepted_status:
            if property_filters:
