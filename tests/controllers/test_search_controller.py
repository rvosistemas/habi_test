...
def test_get_search_successfully():
    filters = {
        "property_filters": {
        ...
        "status_history_filters": {"update_date": "2021-04-12"},
        "status_filter": {"name": "pre_venta", "label": "Inmueble publicado en preventa"},
    }
    ...
    assert data_result[0]["status_label"] == "Inmueble publicado en preventa"
    assert data_result[0]["update_date"] == datetime.datetime(2021, 4, 12, 22, 23, 56)


@pytest.mark.usefixtures("db_connection")
def test_get_search_unsuccessfully():
    filters = {
        "property_filters": {
            "address": "calle 23 #45-67",
            "city": "bogota",
